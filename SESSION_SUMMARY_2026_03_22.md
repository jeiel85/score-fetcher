...
### 6. 공유 모달 헤더 제거 및 풀스크린 개편 (`index.html`)
- 헤더(`🖼️ 공유 미리보기` + 닫기 버튼) 완전 제거
- 이미지가 화면을 꽉 채우도록 풀스크린 구조로 변경
- 푸터 구성: `[닫기]` · `🖼️ 공유 미리보기` · `[공유하기 🚀]`
- 모달 열리면 3초 후 푸터 자동 숨김 (스르륵), 이미지 터치 시 재표시 (타이머 재시작)
- 헤더가 이미지와 겹쳐 보이던 문제 해결
...
| 커밋 | 내용 |
|------|------|
| `e8b888f` | fix: 공유 모달 헤더 제거, 푸터에 통합 |
| `6530d61` | feat: 공유 모달 푸터 3초 후 자동 숨김 + 터치 시 재표시 |
