### `Current Version: v1.5.9` (2026.03.27)

> 최종 목표: 웹 PWA → Capacitor 기반 Android / iOS 네이티브 앱 출시

---

## 현재 상태 (2026-03-26 기준, 최종 업데이트: 22:20)

| 웹 PWA (Vercel 배포) | ✅ [score-fetcher.vercel.app](https://score-fetcher.vercel.app/) |
| **콘티 메이커 브랜딩 리뉴얼** | ✅ 완료 (2026-03-26) |
| **관리자 대시보드 모바일 반응형** | ✅ 완료 (2026-03-26) |
| **Github 토큰 암호화 & 동기화** | ✅ 완료 (2026-03-26) |
| **태블릿 가로 모드 최적화** | ✅ 완료 (2026-03-26) |
| **관리자 모바일 UX & 통계 기능** | ✅ 완료 (2026-03-26) |
| **악보 전체보기(확대) 모달** | ✅ 완료 (2026-03-26) |
| 콘티 저장 / 불러오기 (Firebase) | ✅ 운영 중 |
| Cloud Function (푸시 알림) | ✅ 운영 중 |
| 곡/가사 DB 마이그레이션 | ✅ 완료 |
| Android 앱 (Capacitor) | 🚧 준비 중 (v1.6 목표) |

---

## 최근 작업 이력 (2026-03-27) / v1.5.9 릴리즈 🚀

### 📱 가로/세로 모드 UX 개선 및 이슈 수정 (v1.5.9)
- **#40**: 가로 모드 `ls-v-info`에 관리자(`👑 관리자`) 링크 추가 — 세로 모드 푸터와 동일한 접근성 확보.
- **#41**: 악보가 있는 상태에서 세로→가로 전환 시 분할 뷰 자동 전환 로직 추가 (resize handler 3번째 케이스).
- **#42**: 세로 모드 소형 화면(`max-width: 390px`) 버튼 패딩/폰트 최적화.
- **#43**: 가로 모드 버튼 패딩 절약 (`top-btn: 10px`, `btn-search: 11px`).
- **버전 범프**: `v1.5.8` → `v1.5.9`.

## 최근 작업 이력 (2026-03-27) / v1.5.8 릴리즈 🚀

### 🔐 관리자 접근성 개선 (v1.5.7)
- **버그 수정**: admin.html에서 `firebase-config.js` 누락으로 Google 로그인이 동작하지 않던 문제 수정 (Closes #35).
- **UX**: 메인 앱 푸터에 관리자 페이지 바로가기 링크(`👑 관리자`) 추가 — GitHub 링크 옆 배치.

## 최근 작업 이력 (2026-03-26) / v1.5.6 릴리즈 🚀

### 📊 성능 및 데이터 인사이트 (v1.5.6)
- **✨ 지능형 프리페칭**: IntersectionObserver와 입력창 실시간 감지를 통한 악보 이미지 사전 로드 로직 고도화.
- **✨ 관리자 통계 탭**: 가장 많이 불린 찬양 TOP 10 및 월별 활동 추이 시각화 (Chart.js 도입).
- **📱 관리자 모바일 UX**: 테이블 레이아웃을 카드형으로 가변시키는 반응형 CSS 적용으로 모바일 가독성 혁신.

### 📱 가로 모드 및 악보 뷰어 혁신 (v1.5.0 ~ v1.5.5)
- **✨ 가로 모드 UX 최종 보정**: 전문가 피드백을 반영하여 글로벌 푸터를 제거하고 버전 정보를 앵커형으로 배치. 가독성을 위해 폰트 및 여백 미세 조정.
- **✨ 악보 확대 기능**: 관리자 페이지 및 메인 뷰어에서 악보를 원본 크기로 확대해서 볼 수 있는 줌 모달 구현 및 레이어 간섭 문제 해결.
- **🍎 인터랙티브 회전 대응**: 세로에서 가로로 회전 시, 보고 있던 악보가 즉시 **전체화면**으로 자동 확대되는 UX 구현.

### 🚨 가사 신고 및 데이터 관리 고도화 (v1.4.0 ~ v1.4.6)
- **✨ 통합 신고 시스템**: 사용자가 제보한 가사 오류를 관리자가 대시보드에서 즉시 확인, 수정, 완료 처리할 수 있는 워크플로우 완성.
- **🛠️ 관리자 편의성**: 악보 vs DB 교차 진단기, GitHub 이미지 직접 푸시 기능, 가사 수정 전용 2단 모달 UI 도입.

### 🔐 보안 및 인프라 강화 (v1.2.0 ~ v1.3.8)
- **기기 간 동기화**: GitHub 토큰을 AES-256 암호화하여 Firebase에 저장, 어떤 환경에서든 즉시 악보 업로드 가능하도록 구현.
- **사용 빈도(♪) 안정화**: 데이터 로딩 중 발생할 수 있는 데이터 구조 불일치 및 예외 처리 강화.
- **🚀 퀵 네비게이션**: 700곡에 육박하는 리스트를 100단위로 즉시 점프하는 Apple 스타일 인덱스 바 구현 및 자동 숨김 UX 적용.

### 🎨 브랜드 및 UX 리뉴얼
- **리브랜딩**: 서비스 명칭을 **"콘티 메이커 (Conti Maker)"**로 확정 및 블랙 테마 프리미엄 디자인 전면 적용.
- **강제 업데이트**: 앱 실행 시 버전 체크를 통한 자동 새로고침 및 버전 배지 표시.

### 🎯 다음 목표 (v1.6 ~)
- [ ] **네이티브 기능**: Capacitor를 이용한 실제 Android 폰에서의 딥링크 테스트 및 네이티브 FCM 플러그인 전환
- [ ] **데이터 관리**: 찬양별 가사/태그 관리자 직접 수정 기능 강화
