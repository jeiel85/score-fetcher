...
### `Current Version: v1.4.4` (2026.03.26)

> 최종 목표: 웹 PWA → Capacitor 기반 Android / iOS 네이티브 앱 출시

---

## 현재 상태 (2026-03-26 기준, 최종 업데이트: 15:20)
...
| **사용 빈도(♪) 집계 안정화** | ✅ 완료 (2026-03-26) |
| **가사 오류 신고 시스템** | ✅ 완료 (2026-03-26) |
| **블랙 테마 아이콘 & 공유 카드** | ✅ 완료 (2026-03-26) |
| 콘티 저장 / 불러오기 (Firebase) | ✅ 운영 중 |
| Cloud Function (푸시 알림) | ✅ 운영 중 |
| 곡/가사 DB 마이그레이션 | ✅ 완료 |
| Android 앱 (Capacitor) | 🚧 준비 중 (v1.5 목표) |

---

## 최근 작업 이력 (2026-03-26) / v1.4.4 릴리즈 🚀
- **✨ 가사 수정 모달 UI 최적화**: 모달 높이를 화면의 85%로 고정하고, 악보 이미지는 영역 내 자동 축소(contain), 가사 입력창은 남은 세로 공간을 모두 채우도록 레이아웃 개선
- **🐛 악보 미리보기 로드 버그 수정**: 곡 번호 숫자에 따른 3자리 패딩(007.png 등) 및 다양한 확장자 대응 루틴 보강
- **✨ 가사 신고 관리 고도화**: 관리자 페이지에서 신고된 곡의 가사를 즉시 확인 및 수정
...
### 🔐 Github 토큰 암호화 및 클라우드 동기화 구축 — 14:50
- **기기 간 동기화**: `localStorage`에만 머물던 GitHub 토큰을 Firebase RTDB(`settings/`)로 이전하여 로그인만 하면 어떤 환경(회사/집 등)에서든 토큰 재입력 없이 즉시 악보 업로드 가능
- **AES-256 암호화**: 보안을 위해 서버 저장 전 `CryptoJS`를 사용하여 사용자의 UID와 솔트를 기반으로 암호화 처리 후 저장
- **전용 보안 규칙**: 타인이 토큰을 훔쳐볼 수 없도록 `database.rules.json`에 관리자(jeiel85@gmail.com) 전용 읽기/쓰기 권한 명시

### 📱 인터랙티브 반응형 레이아웃 전면 개편 — 09:30
- **태블릿 세로 모드 최적화**: 너비 기준이 아닌 화면 비율(Orientation) 기반으로 레이아웃 전환. 태블릿 세로 시 모바일 뷰 유지
- **회전 시 자동 전환**: 가로 분할 뷰에서 세로로 회전 시, 보고 있던 악보가 즉시 **전체화면**으로 자동 확대되는 사용자 경험(UX) 구현
- **반응형 스위칭**: `js/utils.js` 리스너를 통해 화면 크기 변화에 따른 실시간 UI 재렌더링

### 🛠️ 관리자 대시보드 사용성 개선 — 09:30
- **진단기 제목 매칭**: 이미지가 누락된 곡 표시 시 곡 번호뿐만 아니라 **곡 제목**을 함께 표시하여 식별 용이하게 수정
- **Github 토큰 발급 링크**: 관리자 설정 영역에 토큰 발급 페이지(`settings/tokens`) 바로가기 링크 추가

### 🗄️ 가사 데이터 마이그레이션 및 보존 — 09:30
- [x] **마이그레이션 도구 안정화 (PUT -> PATCH)**: 기존 수동 추가 데이터 보존 및 병합 로직 적용
- [x] **가사 데이터(JSON) 복합 구조 파싱**: `metadata/songs` 구조 완벽 대응 및 전수 적재
- [x] **찬양 목록 UI 정교화**: 가사 버튼 정렬 개선 및 곡 번호 매칭 로직 강화 (001 <-> 1 호환)
- [x] **모달 레이아웃 점핑 방지**: 검색 시 가변 높이 고정 (85vh) 처리

### 🎯 다음 목표
- [ ] **성능 최적화**: 곡 목록 로딩 시 이미지 프리페칭 고도화
- [ ] **네이티브 기능**: Capacitor를 이용한 실제 Android 폰에서의 딥링크 테스트
- [ ] **통계 페이지 추가**: 예배별 찬양 빈도 시각화 차트 도입 예정
